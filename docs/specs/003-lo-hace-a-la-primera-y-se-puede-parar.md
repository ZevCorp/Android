...
| 321 | La sesión de telemetría de un pedido la abre solo la corrida que se abrió: una corrida rechazada por «ya hay una tarea en curso» no abre sesión, no pisa ni deja en nulo la de la corrida viva, y no manda una sesión propia. | integración ola 1 · B1 |
| 322 | Acabar una corrida limpia siempre, aunque soltar el freno lance una cancelación: la cancelación sale, la corrida deja su línea peticion: al acabar y no al abrir la siguiente, y la siguiente se abre suelta. | integración ola 1 · B8 |
...
| 321 | Fuentes de `app` sin comentarios: en toda la app hay una sola llamada a `promptStarted(` (tampoco `::promptStarted`), en `GraphApp.run` y dentro del bloque de `Ejecucion.correr(` (llaves contadas); el `catch` de `CorridaEnCurso` no nombra `Telemetry`; y cada `Telemetry.promptFinished(` de `run` cierra el id que abrió el bloque a través de `telemetryId?.let`, nunca uno que no se abrió |
| 322 | Un `Freno` cuyo `avisa` lanza `CancellationException` al decir «Listo, tienes el control de vuelta.», un `ArmadoDeEjecucion` con su `CuentaDePeticion`: dentro de `correr` una `llamada` y `parar`. `correr` sale con esa `CancellationException` (no se la traga ni la cambia); ANTES de abrir otra corrida ya hay exactamente una línea `peticion: llamadas=1 …`; la corrida no queda en curso ni con el alto echado; con el aviso sano la siguiente se abre y nace suelta, y no deja otra línea `peticion:` |
