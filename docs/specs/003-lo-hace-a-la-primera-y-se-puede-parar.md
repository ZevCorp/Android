...
- El rechazo no llega al modelo: la puerta devuelve `false` y el motor lo traduce a «no se pudo
  ...
  teléfono (promesa 317): el log dice la acción, «ya falló dos veces» y el destino como hash corto; el texto
...
- Hay **una** corrida de fuera por proceso: la segunda (la burbuja y la app principal a la vez) no se abre, dice
  «ya hay una tarea en curso» y no paga nada (318). `GraphApp.run` lo mira antes de sus efectos (memoria,
  contexto pendiente, telemetría) y el armado lo decide de verdad al abrir.
- El log de la ejecución (puerta, tope, cuenta, motor, freno, MCP y workflows) no lleva lo que la persona
  escribe, pide o ve (317), pero la app sigue mandando el prompt por su cuenta: `Telemetry.promptStarted` y
