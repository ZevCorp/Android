...
| 246 | La voz en vivo solo se arranca desde el panel de desarrollador y se conecta con el `device_id` del teléfono al path real del proxy de Graph (`/api/android-live-session`, no la ruta con rewrite), nunca directo a OpenAI ni con una key real en el APK. | B1b |
